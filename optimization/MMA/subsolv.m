...
while epsi > epsimin
  ...
  while residumax > 0.9*epsi & ittt < 200
    ...
    qlam = q0 + Q'*lam ;
    gvec = P*uxinv1 + Q*xlinv1;
    GG = P.*(uxinv2.') - Q.*(xlinv2.'); % Hollis: June 2020 % replaced: GG = P*spdiags(uxinv2,0,n,n) - Q*spdiags(xlinv2,0,n,n);
    dpsidx = plam./ux2 - qlam./xl2 ;
    ...
    if m < n
      ...
      bb = [blam' delz]';
      Alam = spdiags(diaglamyi,0,m,m) + GG*(diagxinv.*(GG.')); % Hollis: June 2020 % replaced: Alam = spdiags(diaglamyi,0,m,m) + GG*spdiags(diagxinv,0,n,n)*GG';
      AA = [Alam     a
            a'    -zet/z ];
      solut = AA\bb;
      dlam = solut(1:m);
